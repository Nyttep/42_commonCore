...
/*   Updated: 2022/03/15 14:43:29 by pdubois          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_walls_and_corners(t_game *game, int x, int y)
{
	if (x + y == 0)
		return (mlx_put_image_to_window(game->mlx, game->win,
				game->top_left->img, 0, 0), 1);
	else if (y == 0 && x == game->col - 1)
		return (mlx_put_image_to_window(game->mlx, game->win,
				game->top_right->img, 100 * x, 0), 1);
	else if (y == game->row - 1 && x == 0)
		return (mlx_put_image_to_window(game->mlx, game->win,
				game->bot_left->img, 0, 100 * y), 1);
	else if (y == game->row - 1 && x == game->col - 1)
		return (mlx_put_image_to_window(game->mlx, game->win,
				game->bot_right->img, 100 * x, 100 * y), 1);
	else if (y == 0)
		return (mlx_put_image_to_window(game->mlx, game->win,
				game->top_wall->img, 100 * x, 0), 1);
	else if (x == 0)
		return (mlx_put_image_to_window(game->mlx, game->win,
				game->left_wall->img, 0, 100 * y), 1);
	else if (y == game->row - 1)
		return (mlx_put_image_to_window(game->mlx, game->win,
				game->bot_wall->img, 100 * x, 100 * y), 1);
	else if (x == game->col - 1)
		return (mlx_put_image_to_window(game->mlx, game->win,
				game->right_wall->img, 100 * x, 100 * y), 1);
	return (0);
}
...
void	ft_find_and_put_sprite(t_game *game, int x, int y)
{
	if (game->map[y][x] == '1')
	{
		if (!ft_walls_and_corners(game, x, y))
